...
export class UserDeviceService{
    ...
    async findOne(macAddress:string):Promise<UserDevice>{
      const device= await this.userdeviceRepository.findOne({where: {macAddress}});
    ...
    async updateLastLogin(macAddress:string):Promise<any>{
         ...
         return await this.userdeviceRepository.save({...device,lastLogin:new Date().toISOString().toString()});
