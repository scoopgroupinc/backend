...
import { SaveRatingInput, UpdateRatingInput } from "../rating/dto/save-rating.input";
...
export class RatingGroupService{
    ...
    async saveRatingGroup(saveRatingInput:SaveRatingInput): Promise<any>{
        try {
            const {raterId,contentId,type,startTime,endTime} = saveRatingInput
            // first check if rateriD and contentId already exist
              if((await this.findRaterAndContent(raterId,contentId)).length!==0) {
                   return new HttpException('Content has already been rated by this user', HttpStatus.NOT_ACCEPTABLE);
                }

               const comment:RatingCommentInput[] = await this.ratingCommentService.saveRatingComment(saveRatingInput.comment);
               const createdAt= new Date().toISOString().toString();
           
              const taggedRatings= await this.assignFinalTagToRatings(saveRatingInput.ratingDetails);
              console.log(JSON.stringify(taggedRatings))
              const ratingEntries = taggedRatings.map(ratings=>(
               ...
               const commentIds:string[] = await comment.map(comment=>(comment.id))
              ...
              return true
            //   saveRatingInput.comment=comment
            //   saveRatingInput.ratingDetails= rates
            //   return saveRatingInput;
           } catch (error) {
               console.log(error)
   ...
   async assignFinalTagToRatings(ratings) {
    const groups = ratings.reduce((groups, item) => ({
        ...groups,
   ...
   async findRaterAndContent(raterId:string,contentId:string):Promise<RatingGroupInput[]> {
       return await this.ratingGroupRepository.find({where:{raterId,contentId}});
