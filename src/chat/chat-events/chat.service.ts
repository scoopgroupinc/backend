...
export class ChatService {
    ...
    getConversation(
        ...
        return from(
            ...
            map((conversation: ConversationDTO) => conversation || undefined)
        )
    }
        ...
        return this.getConversation(creator.id, match.id).pipe(
            switchMap((conversation: ConversationDTO) => {
                ...
                if (!doesConversationExist) {
                    ...
                    return from(
                        this.converationRepository.save(newConversation)
                    )
                }
    ...
    joinConversation(
        ...
        return this.getConversation(userId, matchId).pipe(
            switchMap((conversation: ActiveConversationDTO) => {
                if (!conversation) {
                    console.warn(
                        `No conversation exists for userId: ${userId} and match ${matchId}`
                    )
                    return of()
                }
                const conversationId = conversation.id
                return from(
                    this.activeConversationRepository.findOne({ userId })
                ).pipe(
                    switchMap((activeConversation: ActiveConversationDTO) => {
                        if (activeConversation) {
                            return from(
                                this.activeConversationRepository.delete({
                                    userId,
                                })
                            ).pipe(
                                switchMap(() => {
    ...
    getmessages(conversationId: number): Observable<MessageDTO[]> {
        return from(
            this.messageRepository
                ...
                .where('message.conversation.id=:conversation_id', {
                    conversationId,
                })
